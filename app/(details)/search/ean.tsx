...
export default function EanSearchScreen() {
  ...
  // A manually-typed barcode is a `scan`; the auto-run from a `code` param
  // (camera result / history reopen) must NOT re-record.
  const userInitiated = useRef(false);

  const { mutate, isPending } = useMutation({
    mutationFn: (value: string) => productsApi.search({ ean: value, limit: SEARCH_LIMIT }),
    onSuccess: (data, value) => {
      const exact = data && data.products.length > 0 ? findExactGtinMatch(data.products, value) : undefined;
  ...
  function handleSearch() {
    ...
    setHasMore(false);
    setSearched(false);
    userInitiated.current = true; // typed barcode → record as a scan in onSuccess
    mutate(trimmed);
  }
